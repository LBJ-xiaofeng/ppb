...
class SmsLogic 
{
     ...
     * @param unknown $scene
     */
    public function sendSms($scene , $mobile , $code, $session_id=0,$templateCode='SMS_138072655')
    {
        ...
        if ($mobile != '' && check_mobile($mobile)) {//如果是正常的手机号码才发送
            try {
                $smsSign = tpCache('sms.sms_product');
                $resp = $this->realSendSms($mobile,$smsSign, $smsParam,  $templateCode);
            } catch (\Exception $e) {
