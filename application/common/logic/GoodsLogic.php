...
class GoodsLogic extends Model
{
    ...
    public function getCoupon($goods_id){
      //查询是否存在该商品的优惠券
        $sql='select a.coupon_id,a.goods_id,b.name,b.type,b.money,b.condition,b.send_start_time,b.send_end_time,b.use_type from tp_goods_coupon as a LEFT join tp_coupon as b on a.coupon_id=b.id where b.use_type=1 and b.status=1 and a.goods_id='.$goods_id.' and b.send_start_time<'.time().' and b.send_end_time>'.time();
        $coupon=M('goods_coupon')->query($sql);
        foreach($coupon as $k=>$v){
            $coupon[$k]['send_start_time']=date('Y-m-d H:i',$v['send_start_time']);
            $coupon[$k]['send_end_time']=date('Y-m-d H:i',$v['send_end_time']);
        }
            return $coupon;
    }
    /**
