...
{
    protected $user_id=0;
    const USER_POINT=100; //会员注册成功赠送积分
    ...
    public function login($username,$password){
        if(!$username || !$password){
            $result= array('status'=>0,'data'=>'','msg'=>'请填写账号或密码');
        }
        ...
        if(!$user){
           ...
           $result = array('status'=>-3,'data'=>'','msg'=>'账号异常已被锁定！！！');
            ...
            M('users')->where("mobile",$username)->save(['last_login'=>time(),'last_ip'=>request()->ip()]);
    ...
    public function app_login($username, $password, $capache, $push_id=0)
    {
    	$result = array();
        if(!$username || !$password)
           $result= array('status'=>0,'msg'=>'请填写账号或密码','data'=>'');
        $user = M('users')->where("mobile|email","=",$username)->find();
        if(!$user){
           $result = array('status'=>-1,'msg'=>'账号不存在!','data'=>'');
        }elseif($password != $user['password']){
           $result = array('status'=>-2,'msg'=>'密码错误!','data'=>'');
        }elseif($user['is_lock'] == 1){
           $result = array('status'=>-3,'msg'=>'账号异常已被锁定！！！','data'=>'');
        }else{
            //查询用户信息之后, 查询用户的登记昵称
            $levelId = $user['level'];
     ...
     * @return array
     */
    public function reg($username,$pwd,$pwd2,$email,$first_leader){
        ...
        //验证两次密码是否匹配
        if($pwd2 != $pwd){
            return array('status'=>0,'msg'=>'两次输入密码不一致','data'=>'');
        }
        //验证是否存在用户名
        if(get_user_info($username,2)||get_user_info($email,1)){
            return array('status'=>0,'msg'=>'账号已存在','data'=>'');
        }
        $map['mobile'] = $username;
...
//        $map['openid'] = $openid;
        ...
        $map['pay_points'] = $point['value'];
        $res = M('users')->add($map);
        if($res){
            accountLog($res, 0,$map['pay_points'], '会员注册赠送积分'); // 记录日志流水
            $user = M('users')->where("mobile", $username)->find();
            return array('status'=>1,'msg'=>'注册成功','data'=>$user);
        }else{
            return array('status'=>0,'msg'=>'注册失败','data'=>"");
        }

    }
//获取会员等级（根据积分）
    function getUserPint($point){
        $points=M('user_level')->select();
        if($point>=$points[0]['amount'] and $point<$points[1]['amount']){
            return $points[0];
        }elseif($point>=$points[1]['amount'] and $point<$points[2]['amount']){
            return $points[1];
        }elseif($point>=$points[2]['amount'] and $point<$points[3]['amount']){
            return $points[2];
        }elseif($point>=$points[3]['amount'] and $point<$points[4]['amount']){
            return $points[3];
        }elseif($point>=$points[4]['amount'] and $point<$points[5]['amount']){
            return $points[4];
        }elseif($point>=$points[5]['amount']){
            return $points[5];
        }
    }
     /*
    ...
    {
        if (!$user_id) {
            return array('status'=>-1, 'msg'=>'缺少参数','data'=>'');
        }
        $user = M('users')->where('user_id', $user_id)->find();
        $data=self::getUserPint($user['pay_points']);
        $user['level']=$data['level_name'];
        if (!$user) {
            return false;
        }
        return $user;
    ...
    public function get_order_goods($order_id){
        ...
        return array('status'=>1,'msg'=>'获取成功','data'=>$goods_list);
    }
     ...
     * @return array
     */
    public function adds_comment($add){
        if(!$add['order_id'] || !$add['goods_id']) {
            return array('status'=>0,'msg'=>'非法操作','data'=>'');
        }
        //检查订单是否已完成
        $order = M('order')->field('order_status')->where("order_id", $add['order_id'])->where('user_id', $add['user_id'])->find();
        if($order['order_status'] != 4){
            return array('status'=>0,'msg'=>'该笔订单还未确认收货或已经评价过','data'=>'');
        }
        //检查是否已评论过
        $goods = M('comment')->where(['user_id'=>$add['user_id'],'goods_id'=>$add['goods_id']])->find();
        if($goods)  {
            return array('status'=>0,'msg'=>'您已经评论过该商品','data'=>'');
        }
        $row = M('comment')->add($add);
        if($row)
        {
            //更新订单商品表状态
            M('order_goods')->where(array('goods_id'=>$add['goods_id'],'order_id'=>$add['order_id']))->save(array('is_comment'=>1,array('order_status'=>5)));
            M('goods')->where(array('goods_id'=>$add['goods_id']))->setInc('comment_count',1); // 评论数加一
            return array('status'=>1,'msg'=>'评论成功','data'=>$add);
        }else{
            return array('status'=>0,'msg'=>'评论失败','data'=>'');
        }
    ...
    public function add_address($user_id,$address_id,$data){
        ...
        //检查格式
        if($post['consignee'] == ''){
            return array('status'=>0,'msg'=>'收货人不能为空','data'=>'');
        }
        if(!$post['province'] || !$post['city'] || !$post['twon']){
            return array('status'=>0,'msg'=>'所在地区不能为空','data'=>'');
        }
        if(!$post['address']){
            return array('status'=>0,'msg'=>'地址不能为空','data'=>'');
        }
        if(!check_mobile($post['mobile'])){
            return array('status'=>0,'msg'=>'手机号码格式有误','data'=>'');
        }
//        添加
        if($address_id == 0)
        {
            ...
            if($c >= 20){
                return array('status'=>0,'msg'=>'最多只能添加20个收货地址','data'=>'');
            }
            //添加模式
            $post['user_id'] = $user_id;

            // 如果目前只有一个收货地址则改为默认收货地址
            $c = M('user_address')->where("user_id", $post['user_id'])->select();
            if(count($c) == 0){
                $post['is_default'] = 1;
            }
            //如果设为默认地址
            ...
            if(!$address_id){
                return array('status'=>0,'msg'=>'添加失败','data'=>'');
            }else{
        ...
        if($address_id > 0){
            ...
            if($post['is_default'] == 1 && $address['is_default'] != 1)
             M('user_address')->where(array('user_id'=>$user_id,'address_id'=>$address_id))->save(array('is_default'=>0));
            $row = M('user_address')->where(array('address_id'=>$address_id,'user_id'=> $user_id))->save($post);
            if($row !== false){
                return array('status'=>1,'msg'=>'编辑成功','data'=>'');
            }else{
                return array('status'=>-1,'msg'=>'操作完成','data'=>'');
            }
