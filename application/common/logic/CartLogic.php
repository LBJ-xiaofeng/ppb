...
class CartLogic extends Model
{
    ...
    {
        if(empty($this->goods)){
            return ['status'=>-3,'msg'=>'购买商品不存在','data'=>''];
        }
...
//        }
        ...
        if ($userCartCount >= 20) {
            return array('status' => -9, 'msg' => '购物车最多只能放20种商品', 'data' => '');
        }
        $specGoodsPriceCount = Db::name('SpecGoodsPrice')->where("goods_id", $this->goods['goods_id'])->count('item_id');
        if(empty($this->specGoodsPrice) && !empty($specGoodsPriceCount)){
            return array('status' => -1, 'msg' => '必须传递商品规格', 'data' => '');
        }
        //有商品规格，和没有商品规格
...
//        $cartCheckAfterList = $cartList;
     ...
     */
    public function doUserLoginHandle($uid)
    {
        if (empty($uid)) {
            return;
        }
