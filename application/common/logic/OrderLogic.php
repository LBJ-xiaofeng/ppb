...
class OrderLogic
{
	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
		$order = M('order')->where(array('order_sn'=>$order_id,'user_id'=>$user_id))->find();
		...
		if($order['pay_status'] > 0 || $order['order_status'] !=1){
			return array('status'=>0,'msg'=>'支付状态或订单状态不允许','data'=>'');
		}
		//有余额支付的情况
		...
		$row = M('order')->where(array('order_sn'=>$order_id,'user_id'=>$user_id))->save(array('order_status'=>0));
		...
		if(!$row){
			return array('status'=>0,'msg'=>'取消订单失败','data'=>'');
		}else{
			return array('status'=>1,'msg'=>'取消订单成功','data'=>'');
		}
	}
     ...
     * @return type
     */
    public function delOrder($order_id,$uid)
    {
        if(empty($uid)){
			return ['status'=>0,'msg'=>'非法操作','data'=>''];
		}
        $row = M('order')->where(['user_id'=>$uid,'order_id'=>$order_id])->update(['deleted'=>1,'order_status'=>3]);
        if (!$row) {
	 ...
	 * 自动取消订单
	 */
	public function abolishOrder($uid){
		...
		foreach($order as $key =>$value){
			$result = $this->cancel_order($uid,$value);
		}
		return $result;
	}
	/**
...
//获取具体地址
	function getdress($id){
		$address=M('region2')->where('id='.$id)->field('name')->find();
		return $address['name'];
	}
    /*
     * 订单操作记录
