...
//        }
    ...
    public function ajaxComment(){
        ...
        $list = M('Comment')->alias('c')->join('__USERS__ u','u.user_id = c.user_id','LEFT')->where($where)->order("add_time desc")->select();
        foreach($list as $k=>$v){
            $list[$k]['add_time']=date('Y-m-d',$v['add_time']);
            $list[$k]['img']=unserialize($v['img'])[$k];
        }
        if($list){
              return json_encode(array('status'=>1,'msg'=>'评论获取成功','data'=>$list));
        }else{
              return json_encode(array('status'=>0,'msg'=>'暂无评论','data'=>''));
        }
    }    
