 ...
 * @param unknown $scene
 */
function sendSms($scene , $mobile , $code, $session_id=0,$templateCode)
{
    $smsLogic = new \app\common\logic\SmsLogic;
    return $smsLogic->sendSms($scene , $mobile , $code, $session_id,$templateCode);
}
 ...
 * @return  bool
 */
function accountLog($mobile, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = 0){
    /* 插入帐户变动记录 */
    $account_log = array(
        'mobile'       => $mobile,
...
//        return false;
//    }
    $update = M('account_log')->add($account_log);
    if($update){
...
//获取单个地名
function get_region2($id){
    $res=M('region2')->where(['id'=>$id])->field('name')->find();
    return $res['name'];
}
/*
 * 获取用户地址列表
...
function get_user_address_list($user_id){
    $lists = M('user_address')->where(array('user_id'=>$user_id))->field('address_id,user_id,consignee,province,city,twon,address,zipcode,mobile,is_default')->select();
    foreach($lists as $k=>$v){
        $ress=getAddress($v['province'],$v['city'],$v['twon']);
        $lists[$k]['province']=$ress[0]['name'];
        $lists[$k]['city']=$ress[1]['name'];
        $lists[$k]['twon']=$ress[2]['name'];
    }
    return $lists;
}

//获取具体地址
function getAddress($pid,$cid,$tid){
    $address=M('region2')->where('id in ('.$pid.','.$cid.','.$tid.')')->field('name')->select();
    return $address;
}
...
function confirm_order($id,$user_id = 0){
    ...
    if($order['order_status'] <3){
        return array('status'=>0,'msg'=>'该订单不为待收货状态,不能确认','data'=>'');
    }
//    if(empty($order['pay_time']) || $order['pay_status'] != 1){
//        return array('status'=>0,'msg'=>'该订单未付款，暂不能确定收货','data'=>'');
//    }
    $data['order_status'] = 4; // 已收货
//    $data['pay_status'] = 1; // 已付款
//    $data['shipping_status'] = 2; // 已收货
    $data['confirm_time'] = time(); // 收货确认时间
    $row = M('order')->where(array('order_id'=>$id))->save($data);
    if(!$row){
        return array('status'=>0,'msg'=>'网络错误，请重试','data'=>'');
    }else{
        return array('status'=>1,'msg'=>'已确认收货','data'=>'');
    }

}
    //保存分成日志
    function addDistributionLog($did,$user_id,$sn,$money){
         $add['did']=$did;
         $add['user_id']=$user_id;
         $add['order_sn']=$sn;
         $add['money']=$money;
         $add['add_time']=time();
        $res=M('distribution_log')->add($add);
        if($res){
            return true;
        }else{
            return false;
...
//获取会员等级（根据积分）
function getUserPint($uid){
    $userPoint=M('users')->where(['user_id'=>$uid])->field('pay_points')->find();
    $points=M('user_level')->select();
    if($userPoint['pay_points']>=$points[0]['amount'] and $userPoint['pay_points']<$points[1]['amount']){
        return $points[0]['level_name'];
    }elseif($userPoint['pay_points']>=$points[1]['amount'] and $userPoint['pay_points']<$points[2]['amount']){
        return $points[1]['level_name'];
    }elseif($userPoint['pay_points']>=$points[2]['amount'] and $userPoint['pay_points']<$points[3]['amount']){
        return $points[2]['level_name'];
    }elseif($userPoint['pay_points']>=$points[3]['amount'] and $userPoint['pay_points']<$points[4]['amount']){
        return $points[3]['level_name'];
    }elseif($userPoint['pay_points']>=$points[4]['amount'] and $userPoint['pay_points']<$points[5]['amount']){
        return $points[4]['level_name'];
    }elseif($userPoint['pay_points']>=$points[5]['amount']){
        return $points[5]['level_name'];
    }
}
...
function getOrder($id)
{
    ...
    $name = M('order')->where(['order_sn' => $id])->field('goods_price')->select();
    $count=0;
    foreach($name as $k=>$v){
        $count+=$v['goods_price'];
    }
...
function getOrderGoods($oid){
    ...
    $name = M('order_goods')->where(['goods_sn' => $oid])->field('goods_name,goods_price')->select();
    return $name;
}
//通过订单号返回商品
function getOrderGoodsname($oid){
    if ($oid == '') {
        return false;
    }
    $name = M('order_goods')->where(['goods_sn' => $oid])->getField('goods_name');
    return $name;
}
//    获取客户名称
function getNickname($id)
{
    ...
    $name = M('users')->where(['user_id' => $id])->getField('nickname');
    return $name;
}
//获取客户消费笔数及金额
function getSumOrder($uid){
    if ($uid == '') {
        return false;
    }
    $data=M('order')->where('user_id',$uid)->where('order_status','gt',1)->where('pay_status','eq',1)->field('count(*) as a,sum(goods_price) as b')->find();
    return $data;
}
//获取客户总分成
function getDistributor($uid){
    if ($uid == '') {
        return false;
    }
    $data=M('distribution_log')->where('did',$uid)->getField('sum(money) as b');
    return $data;
}
//获取商品实际单价
function getPrice($id){
    $data=M('goods')->where('goods_id',$id)->getField('shop_price');
    return $data;
}
//获取分类名称
function cate_name($id){
    $data=M('goods_category2')->where(['id'=>$id])->getField('mobile_name');
    return $data;
}
//获取订单号
function getOrderSn($id){
    $data=M('order')->where(['order_id'=>$id])->getField('order_sn');
    return $data;
}
//获取管理人
function getAdmin($id){
    ...
    $data=M('admin')->where('admin_id',$id)->getField('user_name');
    return $data;
}
