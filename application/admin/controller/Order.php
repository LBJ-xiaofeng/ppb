...
class Order extends Base {
    ...
    public function index(){
        ...
        $admin=M('admin')->where(['role_id'=>16])->field('admin_id,user_name')->select();
    ...
    public function ajaxindex(){
        ...
        if(session('role_id')==16){
            $condition['ywy_id']=session('admin_id');
        }
        $order_sn = ($keyType && $keyType == 'order_sn') ? $keywords : I('order_sn') ;
        $order_sn ? $condition['order_sn'] = trim($order_sn) : false;
        
        I('order_status') != '' ? $condition['order_status'] = I('order_status') : false;
        I('ywy_id') != '' ? $condition['ywy_id'] = I('ywy_id') : false;
        ...
        foreach($orderList as $k=>$v){
            ...
            $orderList[$k]['mobile_name']=$data['mobile_name'];
            $orderList[$k]['attr_name']=$data['attr_name'];
            $orderList[$k]['brand_name']=$data['brand_name'];
            $orderList[$k]['adminName']=M('admin')->where('admin_id',$v['ywy_id'])->getField('user_name');
        }
    ...
    public function submineYwy(){
        $data=I('post.');
        $where['order_id']=array('in',$data['order_id']);
        $wheres['ywy_id']=array('gt',0);
        $select=M('order')->where($where)->where($wheres)->field('ywy_id')->count();
        if($select>0){
            return json_encode(array('status'=>0,'msg'=>'选择中存在已分配订单'));
        }
        $res=M('order')->where($where)->save(array('ywy_id'=>$data['ywy_id']));
        if($res){
            return json_encode(array('status'=>1,'msg'=>'分配成功'));
        }else{
            return json_encode(array('status'=>0,'msg'=>'失败，请重试哦！'));
        }
    }
    //确认入库
    public function submineOrder(){
        $oid=I('order_id');
        $res=M('order')->where(['order_id'=>$oid])->save(['order_status'=>1]);
        if($res){
            return json_encode(array('status'=>1,'msg'=>'确认成功'));
        }else{
            return json_encode(array('status'=>1,'msg'=>'确认失败'));
        }
    }
    //兑换订单
    ...
    public function ajaxapply_list(){
        ...
        I('is_shipping') != '' ? $condition['is_shipping'] = I('is_shipping') : false;
        $count = M('order_goods')->where($condition)->count();
        $Page  = new AjaxPage($count,20);
        $show = $Page->show();
        //获取订单列表
        $orderList =M('order_goods')->where($condition)->order('rec_id desc')->limit($Page->firstRow,$Page->listRows)->select();
//        print_r($orderList);die;
        $this->assign('orderList',$orderList);
        $this->assign('page',$show);// 赋值分页输出
        $this->assign('pager',$Page);
    ...
    public function apply_detail(){
        if(IS_POST){
            $oid=I('rec_id');
            $code=I('shipping_code');
            $res=M('order_goods')->where(['rec_id'=>$oid])->save(array('is_shipping'=>1,'shipping_code'=>$code));
            if($res){
                return $this->success('发货成功',U('admin/Order/apply_list'));
            }else{
                return $this->error('发货失败');
            }
        }
        $oid=I('rec_id');
        $sql="select a.*,b.original_img,b.shop_price from tp_order_goods as a left join tp_goods as b on a.goods_id=b.goods_id where a.rec_id=".$oid;
        $order=M('order_goods')->query($sql);
        foreach($order as $k=>$v){
            $order[$k]['original_img']='http://'.$_SERVER['HTTP_HOST'].$v['original_img'];
            if(isset($v['shipping_code']) && $v['shipping_code']>1){
                $Courier=$this->getNumber($v['shipping_code']);
                $this->assign('Courier',$Courier);
                $this->assign('Courierlist',$Courier->list);
            }
        }
    ...
    {
        $orderList = Db::name('order_goods')->field("*,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time")->order('rec_id')->select();
        $strTable ='<table width="500" border="1">';
        $strTable .= '<tr>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;">订单id</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;">订单编号</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">兑换日期</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">兑换人</td>';
    ...
    */
    public function ajaxdelivery(){
        ...
        $condition['order_status'] = array('in','1,2,3');
    ...
    public function detail($order_id){
        ...
        if($adminIds && count($adminIds) > 0){
            ...
            $user = M("users")->field('user_id,nickname,mobile')->where(['user_id'=>$order['user_id']])->find();
        ...
        $contact=M('user_collection')->field('id,contact')->select();
    ...
    public function addprice(){
        ...
        $act=I('act');
        if(IS_POST){
            $money=M('order_money')->where(['id'=>$id,'oid'=>$oid])->getField('money');
             $res=M('order_money')->where(['id'=>$id,'oid'=>$oid])->delete();
                if($res){
                    M('order')->where(['order_id'=>$oid])->setDec('goods_price',$money);
            ...
            if($act=='edit'){
                $data=M('order_money')->where(['id'=>$id,'oid'=>$oid])->find();
    ...
    public function priceHanlde(){
        ...
        if(empty($data['id'])){
            ...
            $res=M('order_money')->where(['oid'=>$data['oid'],'id'=>$data['id']])->save($data);
        }
        if($res){
            M('order')->where(['order_id'=>$data['oid']])->setInc('goods_price',$data['money']);
            return $this->success('操作成功',U('admin/order/detail',array('order_id'=>$data['oid'])));
        }else{
    ...
    public function edit_order(){
        ...
        $order['confirm_time']=date('Y-m-d H:i',$order['confirm_time']);
       	if(IS_POST)
        {
            I('first_start_time')?$order['first_start_time'] = date('Y/m/d',strtotime(I('first_start_time'))):false;// 流程一
            I('first_end_time')?$order['first_end_time'] = date('Y/m/d',strtotime(I('first_end_time'))):false;// 流程一
            $order['first_content'] = I('first_content');// 流程一
            I('seacond_start_time')?$order['seacond_start_time'] = date('Y/m/d',strtotime(I('seacond_start_time'))):false;// 流程二
            I('seacond_end_time')?$order['seacond_end_time'] = date('Y/m/d',strtotime(I('seacond_end_time'))):false;// 流程二
            $order['seacond_content'] = I('seacond_content');// 流程二
            I('third_start_time')?$order['third_start_time'] = date('Y/m/d',strtotime(I('third_start_time'))):false;// 流程三
            I('third_end_time')?$order['third_end_time'] = date('Y/m/d',strtotime(I('third_end_time'))):false;// 流程三
            $order['third_content'] = I('third_content');// 流程三
            I('four_start_time')?$order['four_start_time'] = date('Y/m/d',strtotime(I('four_start_time'))):false;// 流程四
            I('four_end_time')?$order['four_end_time'] = date('Y/m/d',strtotime(I('four_end_time'))):false;// 流程四
            $order['four_content'] = I('four_content');// 流程四
            I('five_start_time')?$order['five_start_time'] = date('Y/m/d',strtotime(I('five_start_time'))):false;// 流程五
            I('five_end_time')?$order['five_end_time'] = date('Y/m/d',strtotime(I('five_end_time'))):false;// 流程五
            $order['five_content'] = I('five_content');// 流程五
            I('six_start_time')?$order['six_start_time'] = date('Y/m/d',strtotime(I('six_start_time'))):false;// 流程6
            I('six_end_time')?$order['six_end_time'] = date('Y/m/d',strtotime(I('six_end_time'))):false;// 流程6
            $order['six_content'] = I('six_content');// 流程6
            ...
            $order['goods_price'] = I('goods_price'); // 价格
            $order['pay_time'] = strtotime(I('pay_time')); // 支付时间
            if($order['pay_time']>0){
                $order['pay_status'] = 1; // 支付状态
            }
            $order['confirm_time'] = strtotime(I('confirm_time')); // 确认时间
            if($order['confirm_time']>0){
                $order['order_status'] = 4; // 确认状态
            }
//            print_r($order);die;
            $order['shipping_code'] = I('shipping_code',0); // 快递单号
            if($order['shipping_code']>0){
                $order['shipping_time'] = time(); // 发货时间
                $order['order_status'] = 3; // 寄出状态
            }
...
//        print_r($shipping);die;
    ...
    public function delivery_info(){
        ...
        if(!$orderGoods || $order['order_status']==0)$this->error('此订单商品取消订单退货或换货');//已经完成售后的不能再发货
        ...
        if(isset($order['invoice_no'])){
            $Courier=$this->getNumber($order['invoice_no']);
            $this->assign('Courier',$Courier);
            $this->assign('Courierlist',$Courier->list);
        }
    ...
    //查看物流
    public function getCourier($number)
    {
        $courierCode=self::COURIERCODE;//产品code
        $host = self::HOST;//api访问链接
        $path = "/kdi";//API访问后缀
        $method = "GET";
        $headers = array();
        array_push($headers, "Authorization:APPCODE " . $courierCode);
        $querys = "no=".$number;  //快递号
        $bodys = "";
    ...
    public function getNumber($number){
        $res=$this->getCourier($number);
        $wl=json_decode($res);
        if($wl->result->deliverystatus==1){
            $wl->result->deliverystatus='在途中';
        }elseif($wl->result->deliverystatus==2){
            $wl->result->deliverystatus='正在派件';
        }elseif($wl->result->deliverystatus==3){
            $wl->result->deliverystatus='已签收';
        }elseif($wl->result->deliverystatus==4){
            $wl->result->deliverystatus='派送失败';
        }
        if($wl->result->issign==0){
            $wl->result->issign='未签收';
        }else{
            $wl->result->issign='已签收';
        }
        return $wl->result;
    }
...
//导出订单
    public function export_order()
    {
    	...
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">下单人</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">下单地址</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">电话</td>';
    	...
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">付款金额</td>';
    	$strTable .= '</tr>';
	    if(is_array($orderList)){
	    	$region	= get_region_list();
	    	foreach($orderList as $k=>$val){
                ...
                $strTable .= '<td style="text-align:left;font-size:12px;">'.$val['province'].$val['city'].$val['twon'].$val['address'].' </td>';
	    		$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['mobile'].'</td>';
...
//        print_r($state);die;
    ...
    public function getcity($id){
        $city =  M('region2')->where(array('id'=>$id))->find();
        return $city['name'];
    }
    //市
    public function provinceToCity(){
        $city=M('region2')->where(array('parent_id'=>I('id'),'level'=>2))->field('id,name')->select();
        return  json_encode(['status'=>1,'data'=>$city]);
    }
    //县区
    public function cityToTwon(){
        $city=M('region2')->where(array('parent_id'=>I('id'),'level'=>3))->field('id,name')->select();
        return  json_encode(['status'=>1,'data'=>$city]);
    }
    /**
     * 添加一笔订单
    ...
    public function add_order()
    {
        $order = array();
        //  获取省份
        $province = M('region2')->where(array('parent_id'=>0,'level'=>1))->select();
//        print_r($province);die;
        //  获取订单城市
        $city =  M('region2')->where(array('parent_id'=>$order['province'],'level'=>2))->select();
        //  获取订单地区
        // $area =  M('region2')->where(array('parent_id'=>$order['city'],'level'=>3))->select();
        if(IS_POST)
        {
        $data['user_id']=I('user_id');  //客户id
        $data['goods_num']=I('goods_num');  //物品数量
        $data['consignee']=I('consignee');  //客户姓名
        $data['mobile']=I('mobile');  //客户电话
        $CartLogic = new OrderLogic();
        $data['province']=$CartLogic->getdress(I('province'));  //客户省
        $data['city']=$CartLogic->getdress(I('city'));  //客户市
        $data['twon']=$CartLogic->getdress(I('twon'));  //客户区县
        $data['address']=I('address');  //客户详细地址
        $data['cat_id']=I('cat_id');   //物品类型id
        ...
        $data['deleted']=0;
            ...
            if($res){
               return $this->success('添加成功',U("Admin/Order/detail",array('order_id'=>$res)));
            }else{
               return $this->error('添加失败');
            }                
        }     
        $goods=M('goods_category')->where(['is_show'=>1])->field('id,mobile_name')->select();
        ...
        // $this->assign('twon',$area);
        return $this->fetch();
    }
    //物品取种类
    public function changeData(){
        $data=M('goods_attr')->where('cat_id',I('id'))->field('goods_attr_id,attr_name')->select();
        $brand=M('brand')->where('cat_id',I('id'))->field('id,name')->select();
        $datas=['attr'=>$data,'brand'=>$brand];
        return json_encode(array('status'=>1,'data'=>$datas));
    }
    ...
    public function orderProcess(){
        if(IS_POST){
            ...
            $data['add_time']=time();
            $count=M('user_collection')->count();
            if($count>6){
                return $this->error('已存在6步');
            }
            $res=M('user_collection')->add($data);
            if($res){
                return $this->success('增加成功',U('Admin/Order/orderProcess'));
            }else{
            ...
            $count=M('user_collection')->count();
            $Page=new Page($count,10);
            $data=M('user_collection')->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function processHandle(){
        if(IS_POST){
            $id=I('id');
            $res=M('user_collection')->where(['id'=>$id])->delete();
            if($res){
