...
class Wechat extends Base 
{
    ...
    public function setting()
    {
        ...
        if(IS_POST){
            if(empty($id)){
                ...
                $file = request()->file('headerpic');
                $info=$this->upload($file);
                $post_data['headerpic'] = $info;
                $row = M('wx_user')->add($post_data);
                $row && exit($this->success("添加成功"));
                exit($this->error("添加失败"));
            }else{
                $datas = M('wx_user')->where(array('id'=>$id))->find();
                $data['web_expires'] = 0;
                $data['create_time'] = time();
                I('post.mchid')?$data['mchid']=I('post.mchid'):$datas['mchid'];
                I('post.apikey')?$data['apikey']=I('post.apikey'):$datas['apikey'];
                I('post.appid')?$data['appid']=I('post.appid'):$datas['appid'];
                I('post.appsecret')?$data['appsecret']=I('post.appsecret'):$datas['appsecret'];
                I('post.wxname')?$data['wxname']=I('post.wxname'):$datas['wxname'];
                I('post.qr')?$data['qr']=I('post.qr'):$datas['qr'];
                I('post.share_ticket')?$data['share_ticket']=I('post.share_ticket'):$datas['share_ticket'];
                $file = request()->file('headerpic');
                if(!empty($file) || isset($file)){
                    $data['headerpic']=$this->upload($file);
                }else{
                    $data['headerpic']=$datas['headerpic'];
                }
                $files = request()->file('w_token');
                if(!empty($files) || isset($files)){
                    $data['w_token']=$this->upload($files);
                }else{
                    $data['w_token']=$datas['w_token'];
                }
                $row = M('wx_user')->where(array('id'=>$id))->save($data);
    ...
    public function upload($file){
        if($file){
            ...
            if($info){
                return $this->request->domain().'/public/upload/qr_code/'.$name;
            }else{
                echo $file->getError();
            }
        }
...
//        exit($post_str);
    ...
    public function select_goods(){
        ...
        //http://#/index.php?m=Home&c=Goods&a=info&id=
