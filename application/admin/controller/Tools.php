...
//                    array_map("unlink", glob($path));
	...
	public function ktcity(){
		...
		if($parent_id == 0){
			...
			$parent = M('ktcity')->where("id" ,$parent_id)->find();
		}
		$names = $this->getParentKtcityList($parent_id);
		if(count($names) > 0){
			$names = array_reverse($names);
			$parent_path = implode($names, '>');
		}
		$region = M('ktcity')->where("parent_id" , $parent_id)->select();
		$this->assign('parent',$parent);
		$this->assign('parent_path',$parent_path);
		$this->assign('region',$region);
		return $this->fetch();
	}
	function getParentKtcityList($parent_id){
		$names = array();
		$region =  M('ktcity')->where(array('id'=>$parent_id))->find();
	...
	public function ktcityHandle(){
		$data = I('post.');
		$id = I('id');
		$referurl =  isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U("Tools/ktcity");
		if(empty($id)){
			$data['level'] = $data['level']+1;
			if(empty($data['name'])){
				$this->error("请填写地区名称", $referurl);
			}else{
				$res = M('ktcity')->where("parent_id = ".$data['parent_id']." and name='".$data['name']."'")->find();
				if(empty($res)){
					M('ktcity')->cache(true)->add($data);
			...
			M('ktcity')->where("id=$id or parent_id=$id")->cache(true)->delete();
			$this->success("操作成功", $referurl);
		}
	}
	//地区配送
   ...
   //          $image=$this->request->file('image');
			...
			// $data['image']=$img;
            $res=M('goods_category')->where(['id'=>$data['id']])->save($data);
            if(!$res){
    		...
    		$id=I('id');
    		$catename=M('goods_category')->where(['id'=>$id])->find();
	...
	public function categoryList(){
		...
		if($parent_id == 0){
			...
			$parent = M('goods_category')->where("id" ,$parent_id)->find();
		}
		...
		$region = M('goods_category')->where("parent_id" , $parent_id)->select();
		$this->assign('parent',$parent);
		$this->assign('parent_path',$parent_path);
		$this->assign('region',$region);
	...
	function getParentcategoryList($parent_id){
		$names = array();
		$region =  M('goods_category')->where(array('id'=>$parent_id))->find();
		array_push($names,$region['name']);
		if($region['parent_id'] != 0){
...
//		print_r($data);die;
		...
		if(empty($id)){
			if(empty($data['mobile_name'])){
				...
				// if($data['parent_id']==0){
				// 	$this->error("第一级不可增加", $referurl);
				// }
				$res = M('goods_category')->where("parent_id = ".$data['parent_id']." and mobile_name='".$data['mobile_name']."'")->find();
				if(empty($res)){
					M('goods_category')->cache(true)->add($data);
					$this->success("操作成功", $referurl);
				}else{
            ...
            // $pid=M('goods_category')->where("id=$id")->getField('parent_id');
            // if($pid==0){
            // 	$this->error("第一级不可删除", $referurl);
            // }
			M('goods_category')->where("id=$id")->cache(true)->delete();
			$this->success("操作成功", $referurl);
		}
	}
