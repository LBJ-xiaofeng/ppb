...
//            'oss'       => '对象存储'
	...
	{
		$param = I('post.');
		$video=$this->request->file('store_user_logo');
		$videos=$this->upload($video);
		$param['store_user_logo']=$videos;
//		print_r($param);die;
    ...
    public function addEditNav()
    {
        $model = D("Navigation");
        if (IS_POST) {
			$num=$model->where(['is_show'=>1])->count();
            if (I('id')){
                $model->where(['id'=>I('id')])->save(I('post.'));
            }else{
				if($num>=4){
					$this->error("已存在四个显示!!!");
				}
...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
     ...
     */        
     public function login_task()
     {
        ...
        $order_id_arr = M('order')->where("order_status = 1 and shipping_time < $time")->getField('order_id',true);
     ...
     function right_list(){
     	...
     	if($name){
     	    ...
     	    $right_list = M('system_menu')->where('is_del',0)->order('id desc')->select();
     	}
     ...
     public function right_del(){
     	...
     	if(!empty($id)){
     		$r = M('system_menu')->where("id in ($id)")->setField('is_del',1);
     		if($r){
     			respose(1);
     		}else{
     			respose('删除失败');
