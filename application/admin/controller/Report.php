...
class Report extends Base{
	...
	public function index(){
		...
		$today['today_amount'] = M('order')->where("add_time>$now AND pay_status=1")->sum('goods_price');//今日交易金额
		$today['today_order'] = M('order')->where("add_time>$now and pay_status=1")->count();//今日已付款订单数
		$today['cancel_order'] = M('order')->where("add_time>$now AND pay_status=0")->count();//今日待付款订单数
        ...
        $res = Db::name("order".$select_year)
            ->field(" COUNT(*) as tnum,sum(goods_price) as amount, FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap ")
            ->where(" add_time >$begin and add_time < $end and pay_status=1")
            ->group('gap')
            ->select();
		foreach ($res as $val){
	...
	public function userTop(){
        ...
        $list = Db::name('order')->alias('o')
            ->field('count(o.order_id) as order_num,sum(o.goods_price) as amount,o.user_id,u.mobile,u.email,u.nickname')
    ...
    public function saleOrder(){
        ...
        $order_count = Db::name('order')->alias('o')->where($order_where)->where('pay_status','1')->count();
//		print_r($order_count);die;
        $Page = new Page($order_count,20);
        $order_list = Db::name('order')->alias('o')
            ->field('o.order_id,o.order_sn,goods_num,goods_price,o.add_time,u.user_id,u.nickname')
            ->join('users u','u.user_id = o.user_id','left')
            ->where($order_where)->where('pay_status','1')
	...
	public function saleList(){
        ...
        $where = "o.add_time>$this->begin and o.add_time<$this->end and order_status>1 ";  //交易成功的有效订单
        if($cat_id>0){
        ...
        $res = Db::name('order_goods')->alias('og')->field('og.*,o.order_sn,o.add_time')
            ->join('order o','og.order_id=o.order_id ','left')
            ->join('goods g','og.goods_id = g.goods_id','left')
	...
	public function finance(){
		$sql = "SELECT sum(goods_price) as goods_amount,FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap";
		$sql .= " from  __PREFIX__order ";
		$sql .= " where add_time>$this->begin and add_time<$this->end AND order_status in (1,2) order by add_time";
		$res = M('order')->query($sql);//交易额
		foreach ($res as $val){
		...
		for($i=$this->begin;$i<=$this->end;$i=$i+24*3600){
			...
			$list[] = array('day'=>$date,'goods_amount'=>$tmp_goods_amount,'end'=>date('Y-m-d',$i+24*60*60));
		}
                rsort($list);
		$this->assign('list',$list);
		$result = array('goods_arr'=>$goods_arr,'time'=>$day);
		$this->assign('result',json_encode($result));
		return $this->fetch();
	}
  ...
  /**
    ...
    public function financeDetail(){
        $end_time = $this->begin+24*60*60;
        $order_where = "o.add_time>$this->begin and o.add_time<$end_time AND o.order_status=5";  //交易成功的有效订单
        $order_count = Db::name('order')->alias('o')->where($order_where)->group('o.order_id')->count();
        $Page = new Page($order_count,50);
        $order_list = Db::name('order')->alias('o')
            ->field('o.*,u.user_id,u.nickname')
            ->join('users u','u.user_id = o.user_id','left')
            ->where($order_where)
            ->group('o.order_id')
            ->limit($Page->firstRow,$Page->listRows)->select();
