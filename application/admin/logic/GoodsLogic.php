...
class GoodsLogic extends Model
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                // $cate=list_to_tree($goods_category,'id','parent_id','child',0);
                // return $cate;
 ...
 // print_r($goods_category2);die;
    ...
    function getSortCategory()
    {
        ...
        $categoryList =  M("GoodsCategory")->cache(true)->getField('id,name,mobile_name,parent_id,level');
        $nameList = array();
        foreach($categoryList as $k => $v)
        {
            $name = getFirstCharter($v['mobile_name']) .' '. $v['mobile_name'];
             $nameList[] = $v['mobile_name'] = $name;
             $categoryList[$k] = $v;
        }
